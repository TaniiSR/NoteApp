...
@JvmOverloads constructor(
    ...
    var emptyViewTitle: String? = null
        set(value) {
            field = value
            if (!emptyViewTitle.isNullOrEmpty()) {
                val tvNoResult = emptyView?.findViewById<TextView>(R.id.tvNoResults)
                tvNoResult?.text = emptyViewTitle
            }
            invalidate()
        }

    var emptyViewTextDescription: String? = null
        set(value) {
            field = value
            if (!emptyViewTextDescription.isNullOrEmpty()) {
                val tvNoResultDes = emptyView?.findViewById<TextView>(R.id.tvNoResultsDes)
                tvNoResultDes?.text = emptyViewTextDescription
                invalidate()
            }
        }
